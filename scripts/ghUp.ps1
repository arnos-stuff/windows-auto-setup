...
if (-not (Test-Path .\.git)) {
    Write-Host "❓📦 Cannot find a .git folder, please make sure you are in the right place.." -ForegroundColor Red
}
...
function checkSemverFile {
    if (-not (Test-Path "$ROOT/.semver.yaml") ) {
        semver init ;
    }
}
...
function incrementTag($tagType, $push = $false) {
    ...
    if (-not $aliases.Contains($tag) ) {
        ...
        if ($push) {
            ...
            git tag $(semver up beta) ;
            Write-Host "✅ Created new beta 📦 version: $tag" -ForegroundColor Green
        }
        
    } elseif ($aliases.release.Contains($tag)) {
        if ($push.IsPresent) {
            git tag $(semver up release) ;
            Write-Host "✅ Created new alpha 📦 version: $tag" -ForegroundColor Green
            git push -u origin master --tag $(semver get release);
            Write-Host "🔼📦 pushed: $(semver get release) to ☁️" -ForegroundColor Magenta;
        } else {
            git tag $(semver up release) ;
            Write-Host "✅ Created new release 📦 version: $tag" -ForegroundColor Green
        }
...
function releaseExtension($tagType = "alpha", $extType = ".exe", $push = $false) {

    $filesToRelease = Get-ChildItem -Recurse $ROOT -File -Name "*$extType" ;
    incrementTag -tagType $tagType -push $push ;
...
function exeRelease($tagType, $push = $false) {
    releaseExtension -tagType $tagType  -extType ".exe" -justPush $push;
}


if (!$tagType) {
    Write-Host "❓Please input a tag type:"
    Write-Host "(1)" -ForegroundColor Red -NoNewline;
    Write-Host " alpha" -ForegroundColor Yellow ;
    ...
    $answer = Read-Host -Prompt "Pick Number/Name" ;
    $answer = $answer.Trim().ToLower();
    if (-not ($answer -in @('1','2','3') -or $answer -in @('1','2','3'))) {
        Exit-PSSession;
    }
    elseif ($answer -in @('1', 'alpha')) {
        $tagType = 'alpha';
    } elseif ($answer -in @('2', 'beta')) {
        $tagType = 'beta';
    } elseif ($answer -in @('3', 'release')) {
        $tagType = 'release';
    } else {
        Write-Host "😕 Did not understand, try tags: alpha, beta, release";
        Exit-PSSession;
...
exeRelease -tagType $tagType -push $justPush.IsPresent